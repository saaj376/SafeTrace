...
import { Coordinate, HeatmapGeoJSON } from '../services/api'
  ...
  useEffect(() => {
    ...
    whenMapReady(() => {
      ...
      if (ratingMode && onSegmentClick) {
        map.current.on('click', 'safety-heatmap', (e) => {
          if (e.features?.[0]?.properties?.segment_id) {
            onSegmentClick(e.features[0].properties.segment_id, e.lngLat)
          }
        })
      }
    })
